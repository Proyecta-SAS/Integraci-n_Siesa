...
1. Confirmar host QA, token y path real de ejecucion del conector 142888.
2. Ejecutar `python -m siesa_payments.cli validate-connector` para validar autenticacion y disponibilidad del conector si el HUB expone metadata.
...
5. Revisar `logs/siesa_payments.jsonl` y comparar payload contra el contrato de Siesa.
